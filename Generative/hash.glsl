...
void main() {
    ...
    if (id.x == -1.) {
        if (id.y == -2.) {
            col = vec3(hash(cell.x));
        }
        else if (id.y == -1.) {
            col = vec3(hash(cell));
        }
        else if (id.y == 0.) {
            col = vec3(hash(vec3(cell, iTime)));
        }
        else {
            col = vec3(hash(vec4(cell, floor(iTime), iTime)));
        }
    }
    else if (id.x == 0.) {
        if (id.y == -2.) {
            col = vec3(hash2(cell.x), 0.);
        }
        else if (id.y == -1.) {
            col = vec3(hash2(cell), 0.);
        }
        else if (id.y == 0.) {
            col = vec3(hash2(vec3(cell, iTime)), 0.);
        }
        else {
            col = vec3(hash2(vec4(cell, floor(iTime), iTime)), 0.);
        }
    } else {
        if (id.y == -2.) {
            col = vec3(hash3(cell.x));
        }
        else if (id.y == -1.) {
            col = vec3(hash3(cell));
        }
        else if (id.y == 0.) {
            col = vec3(hash3(vec3(cell, iTime)));
        }
        else {
            col = vec3(hash3(vec4(cell, floor(iTime), iTime)));
        }
    }
